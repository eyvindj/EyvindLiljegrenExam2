...
#read_in <- "./data"
read_in <- "~/Rprogrammingversioncontrol/exam2/EyvindLiljegrenExam2/Data/BRACE_biodex_isokinetic_dynamometer_RAW_A_BID-4_20230415"
read_in2 <- "~/Rprogrammingversioncontrol/exam2/EyvindLiljegrenExam2/Data/BRACE_biodex_isokinetic_dynamometer_RAW_A_R2_20231130"
# List all the files from the folder structure.
all_files <- c(list.files(path = read_in, recursive = TRUE, pattern = ".*", full.names = TRUE),list.files(path = read_in2, recursive = TRUE, pattern = ".*", full.names = TRUE))
all_files
...
#lapply(df_lst_long, function(x) all(apply(x, 2, function(col) is.numeric(col))))
...
# Print head of all dfs in the list
map(df_lst_angle, head)
#View(df_lst_angle$A_BID-4_CTM01.txt)
```
...
coord_min_list <- df_lst_angle %>% 
 ...
 #map(.%>%  slice_min(order_by = value, n = 1)) %>% 
 #map(. %>% slice_min(value, n = 1)) %>%
 map(. %>% dplyr::filter(value== min(value))) %>% # extract only min values of each group
...
coord_min_list <- coord_min_list %>% 
  map(. %>% mutate(new_group = rep(c('start', 'end'), length.out = nrow(.))))  %>% # define new column with corresponding labels: c('start', 'end')
...
map(. %>% mutate(segment = rep(1:(nrow(.) %/% 2), each = 2, length.out = nrow(.)))) %>% #lines 164 and 165 are needed for line 167 to work. try to understand why!!!
