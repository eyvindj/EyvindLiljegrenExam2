...
all_files <- __________(path = read_in, recursive = ___, pattern = __________, full.names = ___)
all_files
...
all_files_name <- paste0(__________(all_files, "([A-Z]_)?R2|______"), 
                         '_', 
...
column_names <- c(__________)

# Read in all the files
df_lst <- all_files %>% 
  map(__________, skip = __________) %>% 
  map(. %>% set_names(column_names)) %>% 
  __________(all_files_name)
...
df_lst_long <- df_lst %>% 
  map(. %>% __________(., function(x) gsub(___, ___, x))) %>% # change the wrong separator to '.'
  ___(. %>% __________(is.character,  __________)) %>% # convert from class character to class numeric
  map(. %>% tibble::add_column(seq_along(.)) = 'time') %>% # use you index to create a new column time
  map(. %>% pivot_longer(__________)) # convert to long format 
...
df_lst_angle <- df_lst_long %>% map(. %>% filter(__________))

# Print head of all dfs in the list
map(df_lst_angle, _____)

# Define the peaks and segment of the Angle variable
df_lst_angle <- df_lst_angle %>% 
  ___(. %>% mutate(keep = ______(value, strict = ______))) %>% # define the peaks using peaks() function form splus2R package
  ___(. %>% __________(__________)) %>% # keep only the values defined to be a peak
  ___(. %>% dplyr::mutate(group = cumsum(abs(lag(value, default = first(value)) - _____) > ___))) # define distinct group whenever you observe difference between values > 10.
...
coord_min_list <- df_lst_angle %>% 
  map(. %>% _____(group)) %>% # group your dfs based correct column
  map(. %>% _____(_____(value))) %>% # extract only min values of each group
  __(. %>% ungroup()) # ungroup dfs

# Print head of all dfs in the list
___(coord_min_list, head)
...
coord_min_list <- coord_min_list %>% 
  ___(. %>% _____(new_group = rep(c(_______), times = ___))) %>% # define new column with corresponding labels: c('start', 'end')
